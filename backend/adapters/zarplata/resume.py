"""Zarplata.ru public resume importer, independent from other site adapters."""
...
POLICY = ResumeURLPolicy(
    "zarplata", "zarplata.ru", r"/resume/(?P<id>[0-9a-fA-F]{16,64})/?",
    r"[0-9a-fA-F]{16,64}",
    r"^(?:zarplata\.ru|www\.zarplata\.ru|[a-z0-9-]+\.zarplata\.ru)$",
)
...
class ZarplataResumeExtractor:
    version = "zarplata-resume-v1"
    policy = POLICY
    ...
    async def list_resume_refs(self, page, policy):
        ...
        for index in range(await links.count()):
            ...
            try:
                ref = policy.validate(href if href and href.startswith("http") else f"https://zarplata.ru{href or ''}")
            except ValueError:
                continue
    ...
    async def extract(self, page, ref, policy):
        identity = ResumeIdentity(
            ...
            birth_date=empty_field("identity", "birth_date", unsupported=True),
        ...
        if await page.locator(L.OWNER_EXPERIENCE_CARD).count():
            # The authenticated owner layout exposes complete cards but not
            # the public child hooks. Keep each visible card as a professional
            # duties block and report unsupported fields without parse errors.
            items = page.locator(L.EXPERIENCE)
            for index in range(await items.count()):
                item = items.nth(index)
                if await item.is_visible():
                    experience.append(ResumeExperience(
                        company=empty_field("experience", "company", unsupported=True),
                        position=empty_field("experience", "position", unsupported=True),
                        duties=SourceField(
                        ...
                        source_section="experience",
                        start_date=empty_field("experience", "start_date", unsupported=True),
                    ))
        else:
            items = page.locator(L.EXPERIENCE)
            for index in range(await items.count()):
                ...
                period = await _child_text(item, L.EXPERIENCE_PERIOD, "period")
                ...
                if period.availability is FieldAvailability.PARSE_ERROR:
                    period = empty_field("experience", "period", unsupported=True)
                # Date parsing is intentionally left to the consumer; no guessed locale.
                experience.append(ResumeExperience(
                    company=company, position=position,
                    duties=duties, source_section="experience",
                    start_date=period,
                ))
        skill_field = await list_field(page, L.SKILL, "skills", label="name")
        skills = [ResumeSkill(name=value) for value in (skill_field.value or [])]
        projects = []
        for index in range(await page.locator(L.PROJECT).count()):
            item = page.locator(L.PROJECT).nth(index)
            if await item.is_visible():
        ...
        snapshot = SiteResumeSnapshot(
            ...
            content_hash="0" * 64,
            imported_at=now_utc(), source_updated_at=await updated_at(page, L.UPDATED), identity=identity,
            contacts=contacts, target=target, location=location, experience=experience,
            skills=skills, projects=projects, education=education, languages=languages,
            courses=courses, certifications=certifications, awards=awards,
            portfolio=portfolio, about=about, additional_sections=additional_sections,
...
extractor = ZarplataResumeExtractor()
